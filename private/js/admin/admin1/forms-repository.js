document.addEventListener("DOMContentLoaded", () => {
  ...
  function render(folders, files) {
    ...
    if (currentFolderId) {
      ...
      backBtn.type = "button"; // prevents accidental submit
      backBtn.onclick = (e) => {
        e.preventDefault();
    ...
    folders.forEach((folder) => {
      const itemDiv = createDriveItem(folder.name, "folder", () => {
    ...
    files.forEach((file) => {
      const itemDiv = createDriveItem(file.file_name, "file", () => {
  ...
  function createDriveItem(name, type, onClick, onDelete) {
  ...
  async function addFolder() {
    ...
    await fetch(`${API_BASE}/folders`, {
      ...
      body: JSON.stringify({ name, parent_id: currentFolderId, adminid: "adminave" })
    });
