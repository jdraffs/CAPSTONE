...
// feedbackRoute.js - Enhanced with Visitor Feedback Support
// Add this route to your existing feedbackRoute.js

// POST - Submit visitor feedback (NO transaction validation needed)
router.post("/feedback/visitor", async (req, res) => {
  console.log('🔍 POST /feedback/visitor hit');
  console.log('📦 Request body:', req.body);
  ...
  // Validation
  if (!visitor_name || !visit_purpose || !department || 
      !overall_rating || !processing_time || !staff_assistance || 
      !clarity || !facility) {
    console.log('❌ Missing required fields');
    return res.status(400).json({
  ...
  try {
    ...
    const deptQuery = `SELECT department_id FROM departments WHERE department_name = $1`;
    ...
    if (deptResult.rows.length > 0) {
      ...
      // If department not found, use a default or create new entry
      // For now, we'll use a generic department_id or null
      department_id = null;
    }
    ...
    // Send notification if rating is 2 or below
    if (overall_rating <= 2) {
      await sendLowRatingNotification(department_id, feedback.id, 'visitor');
    }
    
    res.status(201).json({
      success: true,
      message: "Thank you for your feedback!",
      feedback_id: `VFB-${feedback.id}`,
      submitted_at: feedback.created_at
    });
  } catch (err) {
    console.error("❌ Visitor feedback submission error:", err);
...
// GET - Fetch all feedback (students + visitors) for director
router.get("/feedback/director/all-feedback", async (req, res) => {
  console.log('🔍 GET /feedback/director/all-feedback hit');
  const { start_date, end_date, feedback_type } = req.query;
  
  try {
    let studentQuery = `
      SELECT 
        f.feedback_id as id,
        'student' as feedback_type,
        f.transaction_id,
        CASE 
    ...
    let visitorQuery = `
      SELECT 
        id,
        'visitor' as feedback_type,
        NULL as transaction_id,
        visitor_name as student_identifier,
        overall_rating,
        processing_time,
        staff_assistance,
        clarity,
    ...
    if (start_date || end_date) {
      ...
      const conditionStr = whereClause + conditions.join(' AND ');
      studentQuery += conditionStr.replace('created_at', 'f.created_at');
      visitorQuery += conditionStr;
    }
    
    let finalQuery;
    if (feedback_type === 'student') {
      finalQuery = studentQuery + ' ORDER BY f.created_at DESC';
    } else if (feedback_type === 'visitor') {
      finalQuery = visitorQuery + ' ORDER BY created_at DESC';
    } else {
      finalQuery = `(${studentQuery}) UNION ALL (${visitorQuery}) ORDER BY created_at DESC`;
    }
    
    const result = await pool.query(finalQuery, params);
    
    console.log(`✅ Found ${result.rows.length} total feedback items`);
...
// ENHANCED - Update director analytics to include visitor feedback
router.get("/feedback/director/analytics", async (req, res) => {
  console.log('🔍 GET /feedback/director/analytics hit');
  ...
  try {
    let query = `
      SELECT 
        d.department_name,
        d.department_id,
        COUNT(DISTINCT f.feedback_id) as student_feedback_count,
        COUNT(DISTINCT vf.id) as visitor_feedback_count,
        COUNT(DISTINCT f.feedback_id) + COUNT(DISTINCT vf.id) as total_feedback,
        ROUND(
          (COALESCE(AVG(f.overall_rating), 0) * COUNT(f.feedback_id) + 
           COALESCE(AVG(vf.overall_rating), 0) * COUNT(vf.id)) / 
          NULLIF(COUNT(f.feedback_id) + COUNT(vf.id), 0), 
        2) as avg_rating,
        ROUND(AVG(f.processing_time), 2) as avg_processing_time,
        ROUND(AVG(f.staff_assistance), 2) as avg_staff_assistance,
        ROUND(AVG(f.clarity), 2) as avg_clarity,
        ROUND(AVG(f.facility), 2) as avg_facility,
        COUNT(CASE WHEN f.overall_rating <= 2 THEN 1 END) + 
        COUNT(CASE WHEN vf.overall_rating <= 2 THEN 1 END) as low_rating_count
      FROM departments d
      LEFT JOIN feedback f ON d.department_id = f.department_id
      LEFT JOIN visitor_feedback vf ON d.department_name = vf.department_name
    `;
    ...
    if (start_date || end_date) {
      ...
      if (start_date) {
        conditions.push(`(f.created_at >= $${paramIndex} OR vf.created_at >= $${paramIndex})`);
        params.push(start_date);
        paramIndex++;
      }
    ...
    query += ` GROUP BY d.department_id, d.department_name 
               HAVING COUNT(DISTINCT f.feedback_id) + COUNT(DISTINCT vf.id) > 0
...
// Database Migration SQL - Run this to create visitor_feedback table
/*
CREATE TABLE IF NOT EXISTS visitor_feedback (
  ...
  processing_time INTEGER NOT NULL CHECK (processing_time >= 1 AND processing_time <= 5),
  staff_assistance INTEGER NOT NULL CHECK (staff_assistance >= 1 AND staff_assistance <= 5),
  clarity INTEGER NOT NULL CHECK (clarity >= 1 AND clarity <= 5),
  facility INTEGER NOT NULL CHECK (facility >= 1 AND facility <= 5),
  comments TEXT,
  feedback_type VARCHAR(20) DEFAULT 'visitor',
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);

CREATE INDEX idx_visitor_feedback_department ON visitor_feedback(department_name);
CREATE INDEX idx_visitor_feedback_created ON visitor_feedback(created_at);
CREATE INDEX idx_visitor_feedback_rating ON visitor_feedback(overall_rating);
*/