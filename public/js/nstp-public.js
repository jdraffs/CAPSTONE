// nstp-public.js - Fetch and display NSTP posts from backend
...
function createPostHTML(post) {
  ...
  return `
    <article class="nstp-public-post">
      <div class="post-header">
        <div class="post-author-info">
          ...
          <div class="author-details">
            <h3 class="author-name">NSTP Office</h3>
...
function showLoading() {
  postsContainer.innerHTML = `
    <div class="loading-container" style="text-align: center; padding: 60px 20px; color: #605e5c;">
      ...
      <p style="font-size: 16px;">Loading NSTP announcements...</p>
    </div>
  `;
}
  ...
  postsContainer.innerHTML = `
    <div class="empty-state" style="text-align: center; padding: 80px 20px; color: #605e5c;">
      ...
      <p style="font-size: 16px; max-width: 500px; margin: 0 auto;">
        Check back later for updates from the NSTP Office regarding schedules, requirements, and activities.
      </p>
    </div>
  `;
}
...
// Main function to load and display NSTP posts
async function loadNSTPPosts() {
  showLoading();

  try {
    const response = await fetch('http://localhost:3000/api/nstp/posts');
